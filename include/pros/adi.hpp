...
class Port {
  ...
  public:
     ...
     *
     * 	 // Prints the first and second value from the port tuple (The Adi Port. The first value is
     * the Smart Port) printf("Sensor Smart Port: %d\n", sensorSmartPort); printf("Sensor Adi Port:
     * %d\n", sensorAdiPort);
     * }
     * \endcode
     */
...
class AnalogIn : protected Port {
  ...
  public:
     ...
     * EADDRINUSE - The port is not configured as an analog input (e.g. the port has been
     * reconfigured)
     *
...
class DigitalOut : private Port {
  ...
  public:
     ...
     * This function uses the following values of errno when an error state is
     * reached:
     * EADDRINUSE - The port is not configured as a digital output (e.g. the port has been
     * reconfigured)
     *
     * \param value
     *        The value to set the ADI port to
...
class DigitalIn : private Port {
  ...
  public:
     ...
     * This function uses the following values of errno when an error state is reached:
     *
     * EADDRINUSE - The port is not configured as a digital input (e.g. the port has been
     * reconfigured)
     *
...
class Potentiometer : public AnalogIn {
  ...
  public:
     ...
     * when called.
     *
     * Do not use this function when the potentiometer value might be unstable (rotating the
     * potentiometer)
     *
     * This function uses the following values of errno when an error state is
     * reached:
...
class Pneumatics : public DigitalOut {
  ...
  public:
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston('a', false);			// Starts retracted, extends
     * when the ADI port is high pros::adi::Pneumatics right_piston('b', false, true);	// Starts
     * retracted, extends when the ADI port is low
     *
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston({1, 'a'}, false);			// Starts retracted,
     * extends when the ADI port is high pros::adi::Pneumatics right_piston({1, 'b'}, false, true);
     *	    // Starts retracted, extends when the ADI port is low
     *
     *   pros::Controller master(pros::E_CONTROLLER_MASTER);
     *
     *   while (true) {
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics piston({1, 'a'}, false);            // Starts retracted, extends when
     * the ADI port is high
     *
     *   pros::Controller master(pros::E_CONTROLLER_MASTER);
     *
     *   while (true) {
